...
resource "oci_nosql_table" "database" {
    compartment_id = var.compartment_ocid
    name = random_uuid.database_name.result
    ...
    ddl_statement = "CREATE TABLE IF NOT EXISTS ${random_uuid.database_name.result} (id STRING AS UUID GENERATED BY DEFAULT, createdAt TIMESTAMP(0), collectedAt TIMESTAMP(0), temperature FLOAT, pressure FLOAT, lux INTEGER, PRIMARY KEY (id))"
    table_limits {
        max_read_units = "25"
        max_write_units = "25"
...
resource "oci_functions_application" "function_application" {
  ...
  config = {
    "TABLE_NAME" = random_uuid.database_name.result
