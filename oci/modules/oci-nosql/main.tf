resource "oci_nosql_table" "database" {
    ...
    ddl_statement = "CREATE TABLE IF NOT EXISTS enviro(createdAt TIMESTAMP(0), heading FLOAT, accelerometer STRING, temperature FLOAT, pressure FLOAT, rgb STRING, lux INTEGER, PRIMARY KEY (createdAt))"
    table_limits {
        max_read_units = "25"
        max_write_units = "25"
